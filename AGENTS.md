...
3. `MINOR_ISSUES.md` — накопленные мелкие замечания и техдолг
4. Последний файл в `ai_code_reviews/` — замечания

## Работа с замечаниями ревью
- При запросе "исправить замечания" обязательно проверять и исправлять замечания из двух источников:
  - последний файл в `ai_code_reviews/`
  - `MINOR_ISSUES.md`
- Если пункт уже неактуален, помечать это явно в `MINOR_ISSUES.md` с коротким объяснением.
