...
4. `MINOR_ISSUES.md` — backlog мелких рекомендаций; по каждому пункту фиксировать приписку: `исправил/не исправил` + причина + дата.

Примечание по именованию: если в запросе встречается `MINIR_ISSUES`, использовать `MINOR_ISSUES.md` (фактический файл в репозитории).
...
6. Для фиксов использовать понятный префикс коммита `FIX:` и кратко перечислять, что пофикшено (например: `FIX: add NewsArticle schema and detail page cache headers`).
