...
### 18. Двойной импорт config в cf-translator и libre-translator
- **Файлы:** `src/lib/server/services/cf-translator.ts:1`, `src/lib/server/services/libre-translator.ts:1`
- **Суть:** Translator-ы импортируют конфиг напрямую для дефолтных параметров конструктора, хотя `createTranslationService()` уже передаёт значения явно. Двойная зависимость.
- **Исправление:** Убрать импорт из translator-ов, сделать параметры конструктора обязательными.
- **Источник:** Stage 4 Review (15.02.2026)
- **Статус:** Готово
- **Приписка Antigravity (15.02.2026):** Исправил. Убрал двойной импорт конфига, сделал параметры конструктора обязательными.

### 19. Dead code — `buildGlossaryInstruction()`
- **Файл:** `src/lib/server/services/glossary.ts:79-85`
- **Суть:** Функция экспортируется, но нигде не вызывается.
- **Исправление:** Удалить или пометить комментарием для Stage 7.
- **Источник:** Stage 4 Review (15.02.2026)
- **Статус:** Готово
- **Приписка Antigravity (15.02.2026):** Исправил. Функция `buildGlossaryInstruction` удалена за ненадобностью.

### 20. `protectGlossaryTerms` — двойной проход по строке
- **Файл:** `src/lib/server/services/glossary.ts:57-58`
- **Суть:** `pattern.test()` + `replace()` — два полных прохода. Можно один `replace` + проверка изменения.
- **Источник:** Stage 4 Review (15.02.2026)
- **Статус:** Готово
- **Приписка Antigravity (15.02.2026):** Исправил. Переписал `protectGlossaryTerms` на один проход через `replace` с проверкой изменения.

### 21. Нет тестов для CloudflareTranslator
- **Файл:** `src/lib/server/services/translation-service.test.ts`
- **Суть:** Основной провайдер перевода не покрыт тестами (только LibreTranslator и Chain).
- **Источник:** Stage 4 Review (15.02.2026)
- **Статус:** Готово
- **Приписка Antigravity (15.02.2026):** Исправил. Добавил набор тестов для `CloudflareTranslator` (single, batch, errors) с моком fetch.

### 22. Кнопка «Показать оригинал» — нет aria-pressed
- **Файл:** `src/routes/news/[id]/+page.svelte:98-103`
- **Суть:** Toggle-кнопка без `aria-pressed` для screen readers.
- **Источник:** Stage 4 Review (15.02.2026)
- **Статус:** Готово
- **Приписка Antigravity (15.02.2026):** Исправил. Добавил атрибут `aria-pressed` к кнопке переключения языка.

### 23. Индикатор перевода неотличим от русского оригинала
- **Файл:** `src/lib/components/NewsCard.svelte:12`
- **Суть:** Переведённые и русские статьи показывают одинаковый 🇷🇺. Пользователь не отличит машинный перевод.
- **Исправление:** Для переведённых показывать `🇬🇧→🇷🇺` или иконку перевода.
- **Источник:** Stage 4 Review (15.02.2026)
- **Статус:** Готово
- **Приписка Antigravity (15.02.2026):** Исправил. Теперь для переведённых статей отображается `🇬🇧→🇷🇺`.

### 24. `/api/health` падает на `toISOString()` для `latestArticleAt`
- **Файл:** `src/routes/api/health/+server.ts:17`
- **Суть:** При локальном запуске endpoint возвращает ошибку из-за вызова `toISOString()` на значении, которое приходит не как `Date` (например, строка из БД). В результате health-check отдаёт `{ status: 'error' }`, хотя БД доступна.
- **Исправление:** Нормализовать `latestArticleAt` в `Date` перед сериализацией или добавить type guard/проверку на `instanceof Date`.
