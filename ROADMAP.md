...
#### 4.1 Сервис перевода
- [x] Создать `src/lib/server/services/translation-service.ts`:
  ```typescript
  ...
  ```
- [x] Реализация #1: **Cloudflare Workers AI** (NLLB-200)
...
- [x] Реализация #2 (fallback): **LibreTranslate** (self-hosted)
  - Env: `LIBRETRANSLATE_URL` (default: `http://localhost:5000`)
- [x] Обработка ошибок: если перевод не удался — сохранять оригинал, логировать, ставить `is_translated = false`

#### 4.2 Интеграция перевода в pipeline
- [x] В `feed-fetcher.ts` после сохранения статьи:
...
- [x] В `+page.server.ts`: возвращать `translated_title ?? title`, `translated_snippet ?? content_snippet`
- [x] В `NewsCard.svelte`: показывать индикатор языка (🇷🇺/🇬🇧) если перевод доступен
- [x] На детальной странице: кнопка "Показать оригинал" / "Показать перевод"
...
- [x] Создать `src/lib/server/services/glossary.ts` — словарь терминов, которые НЕ нужно переводить:
  - `JavaScript`, `TypeScript`, `React`, `Svelte`, `Kubernetes`, `Docker`, `API`, `CI/CD` и т.д.
- [x] Подставлять в промпт перевода: "Сохрани следующие термины без перевода: ..."

#### 4.5 Тесты перевода
- [x] Мокать API, тестировать: успешный перевод, ошибка API, batch, глоссарий
